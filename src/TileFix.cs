...
internal class TileFix : WorldGen
{
    ...
    private static unsafe int SaveWorldTiles(BinaryWriter writer)
    {
        var array = new byte[16];
        for (var i = 0; i < Main.maxTilesX; i++)
        {
            var num = (float)i / (float)Main.maxTilesX;
            Main.statusText = Lang.gen[49].Value + " " + (int)(num * 100f + 1f) + "%";
            int num2;
            for (num2 = 0; num2 < Main.maxTilesY; num2++)
            {
                var tile = Main.tile[i, num2].ptr;
                var position = 4;
                byte b2;
                byte b;
                byte b3;
                ...
                if (tile->wire()) b3 = (byte)(b3 | 2u);
                if (tile->wire2()) b3 = (byte)(b3 | 4u);
                if (tile->wire3()) b3 = (byte)(b3 | 8u);
                var num4 = tile->halfBrick() ? 16 : tile->slope() != 0 ? (tile->slope() + 1) << 4 : 0;
                b3 = (byte)(b3 | (byte)num4);
                if (tile->actuator()) b2 = (byte)(b2 | 2u);
                if (tile->inActive()) b2 = (byte)(b2 | 4u);
                if (tile->wire4()) b2 = (byte)(b2 | 0x20u);
                if (tile->wall > 255)
                {
                ...
                if (tile->invisibleBlock()) b = (byte)(b | 2u);
                if (tile->invisibleWall()) b = (byte)(b | 4u);
                if (tile->fullbrightBlock()) b = (byte)(b | 8u);
                if (tile->fullbrightWall()) b = (byte)(b | 0x10u);
                var num5 = 3;
                if (b != 0)
                {
                    b2 = (byte)(b2 | 1u);
                ...
                short num6 = 0;
                var num7 = num2 + 1;
                var num8 = Main.maxTilesY - num2 - 1;
                /*
