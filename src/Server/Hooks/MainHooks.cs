...
public class MainHooks : Main
{
    ...
    private static void OnUpdate(On.Terraria.Main.orig_Update orig, Main self, object _gameTime)
    {
        var gameTime = (GameTime)_gameTime;
        if (!IsEnginePreloaded) IsEnginePreloaded = true;
    ...
    protected static void DoUpdate(Main self, ref GameTime gameTime)
    {
        if (GameTimeSync == 60)
        {
        ...
        if (!GlobalTimerPaused) GlobalTimeWrappedHourly = (float)(gameTime.TotalGameTime.TotalSeconds % 3600.0);
        UpdateCreativeGameModeOverride();
        UpdateWorldPreparationState();


        for (var num = DelayedProcesses.Count - 1; num >= 0; num--)
        {
            var enumerator = DelayedProcesses[num];
            if (!enumerator.MoveNext()) DelayedProcesses.Remove(enumerator);
        }
        ...
        if (ActivePlayerFileData != null) ActivePlayerFileData.UpdatePlayTimer();
        Netplay.UpdateInMainThread();
        gameInactive = !self.IsActive;

        self._worldUpdateTimeTester.Restart();
        if (!WorldGen.gen) WorldGen.destroyObject = false;
        if (gameMenu) mapFullscreen = false;
        ...
        if (LocalGolfState != null) LocalGolfState.Update();
        ...
        if (!MintServer.Config.Game.StrippedDownMode)
        {
            if (self.ShouldUpdateEntities()) self.DoUpdateInWorld(self._worldUpdateTimeTester);
        }
        else
        {
            UpdateServer();
